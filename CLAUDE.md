...
- `gallery.js` — מודול ES של גלריית ה-WebGL האינסופית (עיבוד של דמו Codrops של lhbizarro/infinite-webl-gallery, OGL מ-esm.sh). הסטריפ (`.gal-strip`) מגדיר את פריסת הלופ; הקנבס מרנדר. גלילת עמוד מזינה מהירות + עיוות sine; גרירה בעכבר סורקת. בלי WebGL/‏reduced-motion — הסטריפ נשאר גלוי כקולאז' סטטי (fallback).

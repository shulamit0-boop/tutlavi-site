...
## KidTime — מגביל זמן מסך לילדים (תת-פרויקט, ספטמבר 2026)
תיקיית `kidtime/` היא **פרויקט נפרד לגמרי מהאתר** — אפליקציית Python + Tkinter שרצה על
מחשב Windows ביתי ומגבילה את זמן המסך של הילדים (20 דקות ליום לכל אחד/ת כברירת מחדל).
היא לא נפרסת ל-Vercel ולא קשורה ל-tutlavi.com בשום צורה; `.vercelignore` לא מושפע כי
Vercel מגיש קבצים סטטיים מהשורש בלבד.
- הפעלה: `python -m kidtime` · התקנה: `install-windows.ps1` (משימה מתוזמנת בשם `KidTime`)
- נתונים: `%LOCALAPPDATA%\KidTime\state.json` — ילדים, שימוש יומי, בקשות, קוד הורים מגובב
- לוגיקת הזמן ב-`kidtime/store.py` נבדקת ללא Tk; בדיקות הממשק דורשות תצוגה (`xvfb-run`)
- חילוץ חירום: `python -m kidtime --stop <קוד>` (לא דורש Tk ולא מנהל), ואז `הסרה.bat`
- מערכת שלא הוגדרה (בלי קוד/ילדים) **לא** נועלת — רק אשף, בלי קיוסק. באג מ-8.9.2026.
- **חלון בטיחות** (`grace.py`): אחרי כל הדלקת מחשב, דקה פתוחה לפני הנעילה, עם "השהיה
  למבוגרים". ניתן פעם אחת לכל הדלקה (`claim_boot_grace` מול `winsys.boot_stamp()`) —
  אחרת סגירת התוכנה הייתה מייצרת זמן חופשי אינסופי. אחרי האשף: רבע שעה פתוחה.
- התיעוד המלא: `kidtime/README.md`

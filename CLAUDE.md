...
- `challah.html` — **ערכת כרטיסיות להפרשת חלה** → `tutlavi.com/challah`. 16 כרטיסיות (מרכיבי העיסה, שלבי העבודה ורגעי המעמד) + דף מנחה עם סדר הערב, שיעורי ההפרשה, נוסח הברכה ומתכון בסיס. עמוד עצמאי — לא מקושר מהאתר, `noindex` (למחוק את התג כדי לפרסם). כל התוכן ב-HTML; ה-JS רק שומר העדפות (`localStorage: tut-challah-prefs`). **הדפסה:** `@page A4 margin:0`, A6 (4 בעמוד) או A5 (2 בעמוד, פריסת שני טורים) — המידות במ״מ מוגדרות רק בתוך `@media print`. טווחי מספרים עטופים ב-`.rng` כי מקף בין ספרות מתהפך בטקסט RTL. פונטים: Anomalia לכותרות, Frank Ruehl לגוף (מ-`fonts/` — כאן הם כן בשימוש).
